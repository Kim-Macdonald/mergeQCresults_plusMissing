...
for dir_path, dir_names, file_names in os.walk(os.path.join(cwdPath, file3Path)):
    for f in file_names:
        if fnmatch.fnmatch(f, '*_lineage_report.csv'):
            ...
            df_lineage_combo1 =  [df_lineage_split, df_lineage.iloc[:, 1:8]]
...
df_ncov1 = df_ncov_variant_lineage_artic_merge.iloc[:, 0:18]    #ncovtoolsQC: sample - watch mut'ns
#print(df_ncov1)
df_variant1 = df_ncov_variant_lineage_artic_merge.iloc[:, 19:23]  #ncovwatch: watchlist_id - proportion mut'ns (4 cols)
#print(df_variant1)   
df_variant2 = df_ncov_variant_lineage_artic_merge.iloc[:, 36:37]    #new col: Total Mut'ns (not added below) 
#print(df_variant2)
df_lineage1 = df_ncov_variant_lineage_artic_merge.iloc[:, 29:30]    #pangolin: lineage note
df_lineage2 = df_ncov_variant_lineage_artic_merge.iloc[:, 26:27]    #pangolin: pangoLEARN version
#print(df_lineage1)
#print(df_lineage2)
df_artic5 = df_ncov_variant_lineage_artic_merge.iloc[:, 31:36]  #artic: pct_N_bases - qc_pass_y (5 cols)
df_artic6 = df_ncov_variant_lineage_artic_merge.iloc[:, 30:31]  #artic: sample_name
#print(df_artic5)
#print(df_artic6)
